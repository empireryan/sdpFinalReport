...
\todo[inline]{do not write out efficiency estimate in math mode. Say eta with the appropriate subscript, then explain that variable in text}
...
Input and output connection points were placed around the perimeter of the board for easy interfacing with cables. Eagle PCB was utilized for this design and the board house Advanced Circuits was used for board fabrication. Parts were sourced from the Digikey\todo[inline]{just do me a favor and read this}. The top layer of the PCB is shown in Figure ~\ref{PCB top} and the bottom layer in Figure ~\ref{PCB bottom}. The completed circuit board is displayed in Figure ~\ref{hybrid_PCB}.\todo[inline]{really need to work on the wording of this paragraph} 
